...
knitr::opts_knit$set(root.dir = '~/scripts/ConsensusML/composite_code/rnotebook/')
...
```{r}
counts <- read.csv("data/TARGET_NBL_AML_RT_WT_HTSeq_Counts.csv",row.names = 1)
...
AML.CDE.s <- AML.CDE %>%
  ...
  dplyr::select(-matches("^ENSG")) %>% #remove the Norm Counts - since later on these become an issue with DE analysis results 
...
# save(DE.RG, file="data/TARGET_AML_High.Std.Risk_vs_LowRisk_DEGs.rda")
```
...
volcano_plot(fit = DE.RG$fit,cut.off = 5.0)
# dev.off()
```
...
```{r}
fn <- function(col){ifelse(grepl("Unknown", col, ignore.case = TRUE), NA, col)}
dat <- AML.CDE.s %>% 
  filter(Risk.group != "Unknown") %>% 
  mutate(BRG=RiskGroup.Class) %>% 
  dplyr::select(BRG,Gender, Age.at.Diagnosis.in.Days,WBC.at.Diagnosis, Bone.marrow.leukemic.blast.percentage...., Peripheral.blasts....,FAB.Category,FLT3.ITD.positive.,CEBPA.mutation, NPM.mutation,matches("inv.16|^t\\.|del")) %>% 
  mutate_if(.predicate = is.character, .funs = fn) %>% 
  mutate_at(vars(FLT3.ITD.positive.), funs(ifelse(.=="NO", "No", .))) %>% 
  mutate_at(vars(Bone.marrow.leukemic.blast.percentage....), funs(as.numeric(as.character(.))))
...
```{r}
# sapply(dat, table, useNA="always")
```

```{r}
comp <- compareGroups(BRG ~ ., data = dat,method=4,Q1=0, Q3=1, include.miss = "No")
```
...
```{r}
# tab
export2word(tab, "/Users/jlsmith3/ConsensusML/composite_code/rnotebook/data/Binary_Risk_Group_Clin_Characteristics.docx")
```
...
AML.CDE.s <- AML.CDE.s %>% 
  filter(RiskGroup.Class != "Unknown") %>% 
  mutate(BRG=RiskGroup.Class) %>% 
  mutate(Event.ID=ifelse(First.Event == "Censored", 0, 1)) %>% 
  mutate(OS.ID=ifelse(Vital.Status == "Alive", 0, 1))
```

```{r}
fit <- survfit(Surv(Overall.Survival.Time.in.Days/365.25, OS.ID) ~ BRG, data = AML.CDE.s)
...
km <- ggsurvplot(
  ...
  palette = 
    c("blue", "red"),# custom color palettes
  conf.int = TRUE,          # Add confidence interval
  pval = TRUE,              # Add p-value
  risk.table = TRUE,        # Add risk table
  risk.table.col = "strata",# Risk table color by groups
  # surv.median.line="hv",
  # facet.by = "train_test_set",
  ...
  font.x=14,
  font.y=14,
  title = "Overall Survival in Pediatric AML"
) 


#Use only with faceted data
# km <- km + 
...
tiff("figures/TARGET_AML_KM_plot.tiff",height = 5, width = 6, units = "in", res = 300)
km
dev.off()
```
