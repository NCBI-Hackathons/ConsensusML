...
#---------------
repnum = 0
dim(deg.seset) # [1] 1937  145
deg.seset <- deg.seset[,!(deg.seset$deg.risk=="NA")]
dim(deg.seset) # [1] 1937  137
gene.names = as.character(rowData(deg.seset)$countsdf.id)
var.classifier = deg.seset$deg.risk
glm.df = t(assay(deg.seset))
trainset.names = colnames(assay(deg.seset[,deg.seset$exptset.seahack=="train"]))
testset.names = colnames(assay(deg.seset[,deg.seset$exptset.seahack=="test"]))
#----------------------------
x.genes = gene.names
y = var.classifier
df = glm.df 
...
response <- y
predictors <- x
y <- factor(y); names(y) <- colnames(assay(deg.seset))
x = df[,x.genes]
dim(x)
contrast <- contrasts(y)
grid <- 10^ seq(10,-2, length=100)
...
# gene symbols
lasso.goi.symbols <- rowData(deg.seset[lasso.goi,])$gene.symbol
...
bp.gene.symbols <- lasso.goi.symbols[gene.order]
...
mtext(text="Gene Symbol",side=1,line=2,outer=TRUE,cex=1)
