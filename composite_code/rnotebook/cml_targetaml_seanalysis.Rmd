...
for (i in 1:5000){
  ...
  rl <- base::sample(class.labels, size = sum(degfilt.se$exptset.seahack=="train"), replace = TRUE)
  idx <- which(degfilt.se$exptset.seahack=="train")
  ...
  seset.per$deg.risk[idx] <- rl 
  print(table(seset.per$deg.risk, seset.per$exptset.seahack))
  
  #dataframe to hold the randomized labels for later querying 
  p <- paste("perm", i, sep="_")
  labs <- mutate(labs, !! p :=  seset.per$deg.risk)

}

rm(seset.per, idx,rl, p)
