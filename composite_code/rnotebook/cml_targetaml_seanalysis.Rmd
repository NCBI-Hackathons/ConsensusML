...
hmcol = colorRamp2(breaks, colorRampPalette(c("green1","green2", "green3","green4","black",
                                              "red4","red3", "red2","red1"))(n=length(breaks)))
...
```{r, echo=FALSE}
# tiff("figures/TARGET_AML_TrainingSet_Heatmap.tiff", height = 7, width = 12, units="in", res=300)
draw(hm.train)
# dev.off()
```
...
svmtrain = t(assay(degfilt.se[,degfilt.se$exptset.seahack=="train"]))
svmtrainclasses <- degfilt.se[,degfilt.se$exptset.seahack=="train"]$deg.risk

svmtest = t(assay(degfilt.se[,degfilt.se$exptset.seahack=="test"]))
svmtestclasses <- degfilt.se[,degfilt.se$exptset.seahack=="test"]$deg.risk

# Run 1 -- SVM with linear kernel, no weight filter
svm1 <- runSVM(seed=50, kerneltype="linear", 
               trainset=svmtrain, trainclasses = svmtrainclasses,
               testset = svmtest, testclasses = svmtestclasses)

# Run 2 -- SVM with linear kernel, 50% weight filter
svm2 <- runSVM(seed=50, kerneltype="linear", 
               trainset=svmtrain, trainclasses = svmtrainclasses,
               testset = svmtest, testclasses = svmtestclasses,
               weightfilt = 0.5)

# Run 3 -- SVM with linear kernel, no weight filter
svm3 <- runSVM(seed=50, kerneltype="radial", 
               trainset=svmtrain, trainclasses = svmtrainclasses,
               testset = svmtest, testclasses = svmtestclasses)

# Run 4 -- SVM with linear kernel, 50% weight filter
svm4 <- runSVM(seed=50, kerneltype="radial", 
               trainset=svmtrain, trainclasses = svmtrainclasses,
               testset = svmtest, testclasses = svmtestclasses,
               weightfilt = 0.5)
...
for(i in 1:4){
  sr <- svm.resultslist[[i]]$weightsvect[1,]
...
The example I saw randomly sampled the whole dataset and then selected 1:N for class A and the remaining for class B.
see replicate()

I also need to ONLY change the training set, right? this would be because of the test error should be based on the true labels...
and thus show that these genes are clearly the worse model, than the true model.
...
UGGHHHHH so I cant understand it, but the sample() function run BEFORE the runlass() function causes A HUGE decrease in the variablility of the sampling. It is litterally the same few distributions of the class labels.... 

Also, I totally forgot to uncomment the code that save the results. so Yay. 
...
for (i in 1:5000){
  ...
  #random sampling of the class labels.
  rl <- base::sample(class.labels, size = sum(labs$Set=="train"), replace = TRUE)
  idx <- which(labs$Set=="train")
  
  # #dataframe to hold the randomized labels for later querying 
  p <- paste("perm", i, sep="_")
  labs[,p] <- labs$deg.risk
  labs[idx,p] <- rl 
  
  # print(table(labs[,p], labs$Set))
}

#remove unnecessary objects from envio
rm(idx,rl, p)

#Sanity check 
# View(labs[,1:100])

# write.csv(labs, "permutation_5000_labels.csv", row.names = FALSE)
```
...
# for (i in 1:5000 ){
  ...
  lasso.perm <- runLasso(seset = seset.per, seed =  i*7)
  permutation.res[[p]] <- list(lasso.perm$test.error, lasso.perm$nonzero.coef)
  
  #remove for envio
  rm(lasso.perm,seset.per,i,p)
} 

# saveRDS(permutation.res, "permutation_5000_res.RDS")
```
...
```{r}
test.errors[test.errors < 0.07]
names(permutation.res$perm_106) %in% names(l1.tp$nonzero.coef)
```
...
```{r}
# tiff("test_errors_hist_360x_permutations.tiff", height = 5, width = 5,  units="in", res=200)
hist(test.errors, breaks = seq(0,1, by=0.05), 
...
gene_tally[,paste("perm", 1:5000, sep="_")] <- NA
