...
genemeans.0 <- rowMeans(assay(degfilt.se[,degfilt.se$deg.risk==0]))
genemeans.1 <- rowMeans(assay(degfilt.se[,degfilt.se$deg.risk==1]))
lfc.deg <- log2(genemeans.1/genemeans.0)

message("summary of log2FC")
summary(lfc.deg)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -4.963  -0.594   1.299   1.156   2.252   9.414

length(allgenes[which(lfc.deg < -4)]) # 4
length(allgenes[which(lfc.deg < -2)]) # 72
length(allgenes[which(lfc.deg < 0)]) # 699
length(allgenes[which(lfc.deg > 0)]) # 1238
length(allgenes[which(lfc.deg > 2)]) # 585
length(allgenes[which(lfc.deg > 4)]) # 165

# plot
plot(density(lfc.deg))
hist(lfc.deg, breaks = 50)
plot(lfc.deg,-1*log10(rowData(degfilt.se)$p.adj.bh),
...
lfc.cat <- ifelse(lfc.deg <= -4, "< -4",
                  ifelse(lfc.deg <= -2 & lfc.deg > -4,"< -2",
                         ifelse(lfc.deg <= 0 & lfc.deg > -2, "< 0",
                                ifelse(lfc.deg > 0 & lfc.deg < 2, "> 0",
                                       ifelse(lfc.deg >= 2 & lfc.deg < 4, "> 2",
                                              ifelse(lfc.deg > 4, "> 4","NA"))))))
table(lfc.cat)
# < -2 < -4  < 0  > 0  > 2  > 4 
#  68    4  627  653  420  165

hm_rowanno = rowAnnotation(df = data.frame(log2FC = lfc.cat), 
                       col = list(log2FC = c("< -4" =  "chartreuse", "< -2" = "chartreuse3", "< 0" = "chartreuse4",
                                            "> 0" = "firebrick4", "> 2" = "firebrick3", "> 4" = "red")), 
                       width = unit(1, "cm"))

jpeg(paste0(figs.dir, sys.sep, hmdeg.rowanno.name), 10, 7, units="in", res=400)
draw(hm + hm_rowanno)
dev.off()
