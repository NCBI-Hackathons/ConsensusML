...
#convert ensemble IDs to gene names in the Voom fit object
Ensembl.IDs <- data.frame(ensembl_gene_id=rownames(DGE$fit)) %>% 
  ...
  left_join(., ensembl_ID_conversion(Ensembl.IDs = .$ensembl_gene_id), 
            by="ensembl_gene_id") %>% 
  #1 ensemble ID mapped 2 different gene symbols. for simplicity, pick the first one. 
  filter(!duplicated(ensembl_gene_id)) %>%
  #fill in NA values 
  mutate_at(vars(external_gene_name,hgnc_symbol), funs(ifelse(is.na(.) | . == "", ensembl_gene_id, .))) %>%
  #fix duplicate Gene names. Any duplicates stay ENSG identifier rather than remove. 
  mutate_at(vars(external_gene_name), funs(ifelse(duplicated(.), as.character(ensembl_gene_id), .))) %>% 
  #re-order the genes to be the same as the original input. 
  mutate(ensembl_gene_id=factor(ensembl_gene_id,
                                levels = gsub("\\.[0-9]{1,}", "", rownames(DGE$fit)))) %>% #
