...
runLasso <- function(seset, seed=2019, type.measure="class"){
  ...
  #missclassification: how many predicted classes were incorrect
  testError <- mean(pred.class != y[test.names])
  #difference from predicted label to true label
  log.loss <- LogLoss(y_pred = pred.prob[,1],
                      y_true = as.numeric(as.character(y[test.names]))) 
...
permutation.res <- sapply(col.names, function(p){
  ...
  n <- gsub("perm_","",p) %>% 
    as.numeric()
  if(n == 1 | n %in% seq(0,5000, by=100) ){print(p)}
...
permutation.res <- readRDS("data/svm_permutation_5000_res.RDS")
```
...
```{r warning=FALSE}
#use sapply to create a list of the results of lasso for each permutation of class labels
permutation.lasso <- sapply(col.names[1:10], function(p){
...
# saveRDS(permutation.lasso, "data/lasso_permutation_5000_res.RDS")
# permutation.lasso <- readRDS("data/lasso_permutation_5000_res.RDS")
```
...
abline(v=rep1$test.error, col="red", lwd=2)
# dev.off()
```
...
#create random class labels 
random.labels <- replicate(5000, base::sample(class.labels, 
                                              size = sum(labs$Set=="train"), 
                                              replace = TRUE))
...
#Sanity check 
# View(labs[,1:100])
# write.csv(labs, "data/rf_permutation_5000_labels.csv", row.names = FALSE)
```

```{r warning=FALSE}
#use sapply to create a list of the results of lasso for each permutation of class labels
permutation.rf <- sapply(col.names, function(p){
  ...
  # select training data
  rfdat.train <- as.data.frame(t(assay(seset.per[,seset.per$exptset.seahack=="train"])))
  
  # append binom classes
  rfdat.train$class <- as.factor(seset.per[,seset.per$exptset.seahack=="train"]$deg.risk)
  
  # form test set as with train set
  rfdat.test <- as.data.frame(t(assay(seset.per[,seset.per$exptset.seahack=="test"])))
  rfdat.test$class <- as.factor(seset.per[,seset.per$exptset.seahack=="test"]$deg.risk)
  
  #run RF with the randomly assigned group labels 
  rf <- randomForest(class ~ .,
                     ...
                     proximity = TRUE)
  #Test errors
  pred.class <- predict(rf, rfdat.test,'response')
  pred.prob <- predict(rf, rfdat.test,'prob')
  
  tab <- table(observed = as.factor(rfdat.test$class), predicted = pred.class)
  testError <- mean(pred.class !=  rfdat.test$class)
  log.loss <- LogLoss(y_pred = pred.prob[,1],
                      y_true=as.numeric(as.character(rfdat.test$class)))
                      
  return(list(testError,log.loss))

}, simplify = FALSE)

# saveRDS(permutation.rf, "data/rf_permutation_5000_res.RDS")
```

```{r}
rf.test.errors <- sapply(permutation.rf, "[[", 1)
rf.log.loss <- sapply(permutation.rf, "[[", 2) 
```

```{r}
# jpeg("figures/rf_test_error_hist_5000x_permutations.jpg", height = 5, width = 5,  units="in", res=200)
hist(rf.test.errors,  breaks=seq(0,1, by=0.025), 
      xlab="Test Error", ylab="Number of Occurances", main="Test Errors from Permutation Tests \n with RF Classification")
abline(v=mean(pred.rf2k !=  rfdat.test$class), col="red", lwd=2)
# dev.off()
```

```{r}
# jpeg("figures/ff_logloss_hist_5000x_permutations.jpg", height = 5, width = 5,  units="in", res=200)
hist(rf.log.loss, breaks=seq(0,1.2, by=0.025), 
     xlab="Log Loss", ylab="Number of Occurances", main="Log loss from Permutation Tests \n with RF Classification")
abline(v=LogLoss(y_pred = predict(rf2k, rfdat.test,'prob')[,2],
                 y_true=as.numeric(as.character(rfdat.test$class))), 
       col="red", lwd=2)
# dev.off()
```
