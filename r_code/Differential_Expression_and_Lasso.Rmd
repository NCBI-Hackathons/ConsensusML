...
Note: Since I anticipated using all 3 cancer cohorts - I created the normalized dataset from all 3. 
Ultimately I subset for AML only and used raw counts (60K genes +) for DE analysis
So some genes in TMM counts in the AML.CDE.s were not included in the DE and vice versa. 

*To address I will need to go back the original normalization step and do the norm on the 3 cohorts seperately. *
...
# write.csv(RG.GLM.df, "TARGET_AML_RG_Df.csv")
```
...
# write.csv(coef, "TARGET_AML_Logistic_Lasso_withDEGs_RiskGroup_Associated_Genes.csv")
```
...
MLL.GLM.df <- AML.CDE.s %>%
  select(1:84,MLL.Update, train_test_set, 
         intersect(grep("^ENSG", colnames(.), value=T),
                         rownames(DE.MLL$DEGs))) %>% #missing 84 DEGs .... Will re-do ASAP
...
```{r}
Mod.MLL$cv.fit$lambda.min#0.2848036
Mod.MLL$cv.fit$lambda.1se #0.869749
```

```{r}
Mod.MLL$test.error #0.1395349
```
