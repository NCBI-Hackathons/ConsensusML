...
# fields=c("cases.project.project_id",default_fields('files'), grep_fields('files', "associated_entities"), 
#                          "analysis.analysis_type", "analysis.workflow_type", "analysis.workflow_version")

qfiles <- files(fields=available_fields('files')) %>% 
  filter(~ type == 'gene_expression' & 
           analysis.workflow_type == 'HTSeq - Counts' &
          (cases.project.project_id == "TARGET-AML" | cases.project.project_id == "TARGET-NBL" | cases.project.project_id == "TARGET-WT" | cases.project.project_id == "TARGET-RT")) 
...
# write.table(manifest_df, "Manifest/TARGET_Manifest_RNAseq_Counts.txt", row.names = FALSE, sep="\t", quote=FALSE)
```
...
```{r}
#Add the GDC Client to your R options if youre using this. 
...
```{r}
#download step (GDC Client only recommended for large downloads,like BAM, FQs )
...
`find Expn_Data/ -type f -name '*counts*' -exec mv {} . \;`
...
```{r}
# write.csv(counts, "Expn_Data/TARGET_NBL_AML_RT_WT_HTSeq_Counts.csv", row.names = FALSE)
```
