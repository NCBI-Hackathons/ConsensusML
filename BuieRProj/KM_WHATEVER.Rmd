...
SampleTrain <- read.csv("./data/TARGET_AML_Training_Samples.csv")
SampleTest <-  read.csv("./data/TARGET_AML_Testing_Samples.csv")
```
...
PatientsWithPhenoTypeGenes <- merge(CleanedWithGenes, GroupMatching, by = "TARGET.USI", all.x = TRUE)

```

### Executing SVM to compare expressions among groups

```{r aim1_SVM}

test<- PatientsWithPhenoTypeGenes[PatientsWithPhenoTypeGenes$TARGET.USI %in% as.list(SampleTest),]
svm()
