...
KMDF[KMDF$Risk.group == "Standard",]$Risk.group <- "1"
...
GeneWKM <- merge(KMDF,KMeansReview[,c("TARGET.USI","8_k")], by.x = "TARGET.USI")


```

Notably when merging our data files, one grop is removed. This occures at both k=3 and K=4 and K=8 (other k's not yet tested).

### Testing ASsociation of Outcome and Phenotype groups

```{r}

KMRGfit <- glm(as.factor(Risk.group) ~ as.factor(`8_k`), data = GeneWKM, family = "binomial")
...
colnames(SVMResults$weightsvect)
...
SVM
  ...
  Weights were divded into quartiles deciles, . Genes expressed in the outer two quantiles at the 25%, 20%, 1% and 0.1%. The identified genes in each quantiles are listed below.
  
  


## Aim 3 (low priority, should include in aim 1 in rewrite)
